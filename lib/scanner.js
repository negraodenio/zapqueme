...
export function formatWhatsAppMessage(data) {
  ...
  if (data.vezes_reportada && data.vezes_reportada > 1) {
    texto += `\n🔥 *ALERTA DE REPETIÇÃO (UPSTASH REDIS):*\n⚠️ Esta mensagem já foi pesquisada *${data.vezes_reportada} vezes* por outras pessoas! Golpistas frequentemente testam e disparam o mesmo texto em massa.\n`;
  } else {
    texto += `\n🛡️ *Registro no Banco:* 1ª vez que esta mensagem é verificada no sistema.\n`;
  }
