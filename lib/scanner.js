...
export function getRiscoBar(score, mode) {
  ...
  const color = mode === 'critico' ? '🟥' : (mode === 'moderado' ? '🟨' : '🟩');
  const filled = color.repeat(blocks);
...
export function formatWhatsAppMessage(data) {
  ...
  // Determina categoria de criticidade
  let mode = 'critico';
  ...
  // 🔴 1. VERMELHO: GOLPE / RISCO CRÍTICO
  if (mode === 'critico') {
    const tag = tipo ? ` • *[${tipo}]*` : '';

    let msg = `🚨 *ALERTA CRÍTICO: PROVÁVEL GOLPE*${tag}\n\n`;
    msg += `📊 *NÍVEL DE RISCO:* ${score}/100 [CRÍTICO]\n`;
    msg += `${barra} _Perigo Extremo_\n`;

    if (data.vezes_reportada && data.vezes_reportada > 1) {
      msg += `\n🔥 *Disparo em massa:* Pesquisada *${data.vezes_reportada}x hoje* por outras pessoas!\n`;
    }
    ...
    msg += `📞 *Ligue* para o número habitual da pessoa/banco.\n`;
    msg += `🚫 *Bloqueie* e denuncie este contato.\n`;

    msg += `\n🆘 _Já caiu ou fez o Pix? Responda com *fui vítima* para socorro imediato._`;
    return msg;
  }

  // 🟡 2. AMARELO: SUSPEITO / RISCO MODERADO
  if (mode === 'moderado') {
    const tag = tipo ? ` • *[${tipo}]*` : ' • *[LINK SUSPEITO]*';

    let msg = `⚠️ *ATENÇÃO: RISCO MODERADO*${tag}\n\n`;
    msg += `📊 *NÍVEL DE RISCO:* ${score}/100 [SUSPEITO]\n`;
    msg += `${barra} _Atenção Redobrada_\n`;

    if (data.alerta_link) {
      msg += `\n⚠️ *Alerta técnico:* ${data.alerta_link}\n`;
    } else if (data.explicacao_simples) {
      msg += `\n⚠️ *Atenção:* ${data.explicacao_simples}\n`;
    }
    ...
    msg += `\n💡 *O QUE VOCÊ DEVE FAZER AGORA:*\n`;
    msg += `❌ *Não clique* no link e não pague nada.\n`;
    msg += `🔍 *Consulte apenas* pelo app ou site oficial.\n`;
    msg += `⚠️ Se já clicou, *não insira senhas* nem dados.\n`;
    return msg;
  }

  // 🟢 3. VERDE: SEGURO / LEGÍTIMO
  const tag = tipo ? ` • *[${tipo}]*` : ' • *[CANAL OFICIAL]*';

  let msg = `✅ *ANÁLISE: MENSAGEM SEGURA*${tag}\n\n`;
  msg += `📊 *NÍVEL DE RISCO:* ${score < 10 ? '0' + score : score}/100 [SEGURO]\n`;
  msg += `${barra} _Risco Mínimo_\n`;

  msg += `\n🛡️ *Remetente Verificado:* Padrão legítimo sem sinais de fraude.\n`;

  msg += `\n🔍 *Verificações de segurança:*\n`;
  msg += `✓ Nenhum link suspeito detectado.\n`;
  msg += `✓ Não solicita senhas ou transferências urgentes.\n`;

  msg += `\n💡 *Dica:* Pode prosseguir normalmente. Lembre-se: bancos nunca pedem senhas ou códigos por telefone.`;
  return msg;
}
